...
export class VentaGraficaComponent implements OnInit {
  ...
  ventasSemana(): void {
    const ventasAgrupada = _.map(
        ...
        _.filter(
          ...
          (v: Venta) =>
            moment()
              .add(-7, "days")
              .toDate()
              .getTime() <= new Date(v.fecha).getTime()
        ),
      ...
      (v, i) => ({
        ...
        totalTortillas:
          v.filter(v => v.objeto === "tortilla").length > 0
            ? v
                .filter(v => v.objeto === "tortilla")
                .reduce((v, v1) => ({ cantidad: v.cantidad + v1.cantidad }))
                .cantidad
            : 0,
        totalFrijoles:
          v.filter(v => v.objeto === "frijol").length > 0
            ? v
                .filter(v => v.objeto === "frijol")
                .reduce((v, v1) => ({ cantidad: v.cantidad + v1.cantidad }))
                .cantidad
            : 0
      })
  ...
  ventaTotal(): void {
    const ventasAgrupada = _.map(
      _.groupBy(_.reverse(this.ventasOrigen), i =>
        moment(i.fecha).format("DD/MM/YY")
      ),
      (v, i) => ({
        label: i,
        total: v.map(v => v.cantidad * v.costo).reduce((v, v1) => v + v1),
